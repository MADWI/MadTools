 ...
 * 
 * To use it 1) implement the MicrophoneInputListener interface e.g. class
 * MyAwesomeClass implements MicrophoneInputListener {...} 2) Create the object,
 * e.g. micInput = new MicrophoneInput(this); 3) Implement processAudioFrame in
 * your MyAwesomeClass public void processAudioFrame(short[] audioFrame) {...}.
 * 
...
public class MicrophoneInput implements Runnable {
	...
	public void run() {
		...
		try {
			recorder = new AudioRecord(mAudioSource, mSampleRate,
					mChannelConfig, mAudioFormat, buffer1000msSize);
			recorder.startRecording();

			while (mRunning) {
				int numSamples = recorder
						.read(buffer20ms, 0, buffer20ms.length);
	...
	private int bufferSize(int sampleRateInHz, int channelConfig,
			int audioFormat) {
		int buffSize = AudioRecord.getMinBufferSize(sampleRateInHz,
